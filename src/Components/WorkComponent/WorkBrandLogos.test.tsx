...
import WorkBrandLogos from './WorkBrandLogos';
...
describe('Rendering Work Brand Logos', () => {
  it('Rendering Work Brand Logos', () => {
    const brandLogos = [
    ...
    render(<WorkBrandLogos />);
    const titleEle = screen.getByRole('heading', {
      name: 'Our quest for digital excellence. It starts here.',
    });
    expect(titleEle).toBeInTheDocument();
    const descELE = screen.getByText(
      /As drivers of digital excitement, we contribute to the business transformation of our clients across a wide range of industries./i,
    );
    expect(descELE).toBeInTheDocument();
    const images = screen.getAllByRole('img');
    expect(images.length).toBe(brandLogos.length);
  });
});
